...
function mapImages(rows) {
  return rows.map(row => ({...row, url: resolveImageUrl(row.url)}))
}
...
app.get('/api/products', async (req,res)=>{
  try {
    ...
    res.json({page:pgno,limit:lim,total:countResult.rows[0].total,items:rows.map(mapProduct)})
  } catch(e){res.status(500).json({error:e.message})}
})
...
app.get('/api/products/:id', async (req,res)=>{
  try {
    ...
    res.json({...mapProduct(p.rows[0]),offers:offers.rows,specifications:specs.rows,images:mapImages(images.rows),reviews:reviews.rows,questions:questions.rows})
  } catch(e){res.status(500).json({error:e.message})}
})
...
app.get('/api/search', async(req,res)=>{try {const q=String(req.query.q||'').trim(); if(!q)return res.json({query:'',items:[]}); const {rows}=await pool.query(`SELECT id,name,slug,category,price,old_price,image,rating,review_count FROM products WHERE name ILIKE $1 OR description ILIKE $1 ORDER BY review_count DESC LIMIT 50`,[`%${q}%`]);res.json({query:q,count:rows.length,items:rows.map(mapProduct)})}catch(e){res.status(500).json({error:e.message})}})
